...
/**
 * check-slop.mjs — Anti-AI-Slop linter (HTML/CSS · SVG · Markdown).
 *
 * Enforces the visual/document "AI slop" bans from the anti-ai-slop skill
 * (.claude/skills/anti-ai-slop/SKILL.md §1 + references/*.md) and reports each
 * hit with a file:line location. Deterministic, dependency-free (Node stdlib only).
 *
 * Fired automatically (report-only) by posttool-slop.sh on .html/.htm/.css/.svg
 * writes; run by hand for the full report, and for .md.
 *
 ...
 *
 * Usage:
 *   node .claude/hooks/check-slop.mjs <file> [<file> ...]
 *
 * Exit codes:
...
if (files.length === 0) {
  console.error('[carve-harness:check-slop] usage: node .claude/hooks/check-slop.mjs <file> [<file> ...]');
  process.exit(2);
}
...
for (const file of files) {
  ...
  try {
    ...
    console.error(`[carve-harness:check-slop] cannot read ${file}: ${e.message}`);
    process.exit(2);
  }
  ...
  // md 코드 펜스 안은 건너뛴다. scanCopyTone·scanMdOnly 는 이미 펜스를 존중하는데
  // 여기만 무시해서, "이 단어를 쓰지 마라"를 예시로 보여주는 규칙 문서 자신이 ERROR가
  // 났다(자기참조 오탐). 펜스 규약은 스캔 전체에서 하나여야 한다.
  const scanShared = (line, i, inFence) => {
    if (inFence) return;
  ...
  const scanHtmlCss = (line, i, low) => {
    ...
    // top accent bar
    // `}` must be excluded: the last declaration in a block usually has no `;`,
    // so `border-top:4px solid #2563eb}` captured the brace, parseColor's anchored
    // regex rejected `#2563eb}`, and the bar went undetected.
  ...
  console.log(`\n[carve-harness:check-slop] ${file} [${kind}]`);
...
console.log(`\n[carve-harness:check-slop] 총계: ${totalErrors} error(s), ${totalWarns} warning(s) across ${files.length} file(s)`);
if (totalErrors > 0) {
  console.log('[carve-harness:check-slop] MUST-NOT 위반이 있습니다. 제거 후 재작성하세요.');
  process.exit(1);
}
process.exit(0);
