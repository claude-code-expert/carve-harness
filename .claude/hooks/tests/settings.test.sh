...
# GUARD-02 (matcher), GATE-02 (Stop timeout > 600), CFG-02 (${CLAUDE_PROJECT_DIR} ×11),
...
[ "$(grep -o 'CLAUDE_PROJECT_DIR' "$S" | wc -l | tr -d ' ')" = "11" ] \
  && ok "11x CLAUDE_PROJECT_DIR across hook commands (CFG-02)" \
  || no "11x CLAUDE_PROJECT_DIR (CFG-02)"
