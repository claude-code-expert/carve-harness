...
| 전역 자산 | `~/.claude/` | GSD·Squad·플러그인 | 모든 프로젝트 |
| 원격 MCP | claude.ai 계정 | Canva·Context7 | 계정 연동 |
| 레포 자산 | `<repo>/.claude/` | 훅·에이전트·커맨드·규칙 | 이 프로젝트만 |
...
역할별 전문 에이전트 8종 + 파이프라인. 한국어/영어 트리거 자동 위임 또는 `/squad-*` 명시 호출.

- **위치:** `~/.claude/agents/squad-*.md`(8) + `~/.claude/commands/squad-*.md`(9)
...
| 유형 | 레포 `.claude` | 전역 GSD | 전역 Squad | 플러그인 | 합계 |
|------|:---:|:---:|:---:|:---:|:---:|
| 스킬 | 2 | 67 | 8 | caveman 6·ponytail 6·claude-hud 2 | 91 |
| 에이전트 | 5 | 33 | 8 | cavecrew 3 | 49 |
| 커맨드 | 5 | (스킬로) | 9 | — | 14 |
| 훅 | 7 | — | — | — | 7 |

> + **전역 ECC 선별(2026-07-07):** 에이전트 5·스킬 2 (§4.5) → 스킬 합 93·에이전트 합 54.
