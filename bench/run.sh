...
echo ""; echo "═══ 3) 라이브 비교 (보류 — 셋업·API 필요) ═══"
echo "  축 1(토큰·\$·시간)·3(cross-harness 트리거)·4(컨텍스트)·5(E2E)는 LLM 실행 필요:"
echo "   1. bench/harnesses/README.md 따라 A~E 환경 셋업"
echo "   2. bench/tasksets/{crud,multi,refactor}.md 를 각 하네스로 n>=5 실행"
echo "   3. npx ccusage@latest 로 토큰·\$ 수집 → bench/results/<harness>.json (스키마: report.mjs 참고)"
echo "   4. node bench/report.mjs → 스코어카드(중앙값·IQR) 생성"
