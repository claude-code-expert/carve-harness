...
> - `2026-06-21` **v1.8.0** — 스킬 슬래시 이중 노출(`/<id>`+`/carve-<id>`) 제거: 16개 스킬 디렉터리를 `carve-<id>`로 접두해 슬래시를 `/carve-<id>` 하나로 수렴(내장 충돌 클래스 소멸), 커맨드 shim 레이어 폐지(스킬이 `$ARGUMENTS` 네이티브 지원). 카탈로그 id는 bare 유지, `carve install` 재실행 시 구설치 old 경로+옛 shim 자동 정리(해시 가드)
...
**v1.8.0** · TypeScript(ESM, 빌드 단계 없음) · Node >=22.18 · 테스트 289 / 커버리지 약 88.5%
...
carve install --only commit,handoff,tdd      # 명시 선택 (일괄 설치 없음)
...
| 하고 싶은 것 | 부르는 법 | 무엇이 |
|---|---|---|
| 커밋 메시지 | "커밋 메시지 만들어" · `/carve-commit` | Conventional Commit 생성(빠른 인라인) |
| 코드 리뷰 | "리뷰해줘" · `/squad review` | 키워드 자동 위임 → squad-review |
| 세션 인계 | "핸드오프" · `/carve-handoff` | 진행·결정·다음 할 일을 남겨 다음 세션이 이음 |
| 결정 기억 | "기억해둬" · `/memory` | Claude Code 내장 메모리 |
...
**보안**
- `squad-audit` 보안 감사·취약점 스캔 *(`security-scan` 스킬은 v1.4.0부터 비추천 — squad-audit으로 일원화)*

**릴리스·협업**
- `squad-gitops` 커밋·PR·체인지로그 · `squad-docs` 문서 생성·갱신 · `squad-plan` 기획·유저스토리 *(PR 본문은 내장 `/pr` 또는 squad-gitops — `pr`·`changelog` 스킬은 비활성)*

**문서·시각물 (anti-slop)**
- HTML·SVG·카드뉴스·리포트·슬라이드 생성 시 AI 슬롭(그라데이션·글로우·워터마크 등)을 제거하고 `check-slop`이 결정적으로 게이트 ("슬롭 없는 html 만들어")

**멀티에이전트·비용 최적화**
- `parallel-agents` 3~4 병렬 + git worktree 격리 · `model-route` Haiku/Sonnet/Opus 라우팅 · `evaluator-tuning` 평가자 few-shot 보정(선택 — 직접 선택 시 설치) *(`coordinator`는 비추천 — parallel-agents로 충분)*
