...
**v1.4.0** · TypeScript(ESM, 빌드 단계 없음) · Node >=22.18 · 테스트 262 / 커버리지 약 88.2%
...
**릴리스·협업**
- `pr` PR 본문 · `squad-gitops` 커밋·PR·체인지로그 · `squad-docs` 문서 생성·갱신 · `squad-plan` 기획·유저스토리 *(`changelog` 스킬은 비추천 — squad-gitops가 담당)*
...
- `full` — standard + **추가 스킬**(verify·iterate·test-gen·parallel-agents 등 — 비추천 컴포넌트는 자동 제외)
