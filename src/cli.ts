...
import { cmdList, cmdInstall, cmdDoctor, cmdUninstall } from './commands.ts';
...
// 알려진 명령.
export const KNOWN_COMMANDS = ['install', 'uninstall', 'list', 'doctor'] as const;
...
export function run(args: string[], io: IO = console): number {
  ...
  switch (cmd) {
    case 'list':
      return cmdList(io);
    case 'install':
      return cmdInstall(dir, io, parseOnly(args));
    case 'doctor':
