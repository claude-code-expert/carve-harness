...
test('회귀: --level full 설치 시 manifest.level 영속 + update가 동일 레벨 재현', () => {
  withTemp((root) => {
    ...
    assert.ok(m?.files.some((f) => f.path === '.claude/skills/verify/SKILL.md'), 'full 컴포넌트(verify) 미설치');
    // update가 m.level(full)로 재현 → verify를 신규 추천으로 오판하지 않음
    const { io: io2, cap: cap2 } = captureIO();
    assert.equal(cmdUpdate(root, io2), 0);
    assert.ok(!cap2.logs.some((l) => l.includes('verify') && l.includes('신규 추천')), 'full 컴포넌트가 신규 추천으로 오판');
  });
});
