...
// ── Scenario E: wave-1 deprecated 설치본 하위호환 (fade-out 수용 게이트) ──

test('라이프사이클 E: 구버전 설치(deprecated 포함) → update(보존·동결·안내) → doctor(안내) → uninstall(완전 제거)', () => {
  withTemp((root) => {
    seedProject(root);
    // 1) 현행 설치 + 구버전 설치 재현: wave-1 이전에 깔렸던 deprecated 자산을 수동 추가
    assert.equal(cmdInstall(root, captureIO().io), 0);
    const changelogSkill = '.claude/skills/changelog/SKILL.md';
    const scanSkill = '.claude/skills/security-scan/SKILL.md';
    const changelogContent = '# changelog skill (pre-wave-1 install)\n';
    const scanContent = '# security-scan skill (pre-wave-1 install)\n';
    mkdirSync(join(root, '.claude/skills/changelog'), { recursive: true });
    mkdirSync(join(root, '.claude/skills/security-scan'), { recursive: true });
    writeFileSync(join(root, changelogSkill), changelogContent);
    writeFileSync(join(root, scanSkill), scanContent);
    const m0 = readManifest(root) as Manifest;
    const extra: ManifestFile[] = [
      { path: changelogSkill, hash: hashContent(changelogContent), assetVersion: '1.3.0' },
      { path: scanSkill, hash: hashContent(scanContent), assetVersion: '1.3.0' },
    ];
    writeManifest(root, { ...m0, files: [...m0.files, ...extra] });

    // 2) update: deprecated는 generate()에서 자연 탈락 → 갱신 동결 + 바이트 보존 + 명시 안내
    const up = captureIO();
    assert.equal(cmdUpdate(root, up.io), 0);
    assert.equal(read(root, changelogSkill), changelogContent, 'update가 deprecated 자산을 건드림');
    assert.equal(read(root, scanSkill), scanContent, 'update가 deprecated 자산을 건드림');
    const upLog = up.cap.logs.join('\n');
    assert.match(upLog, /갱신 동결\(비추천\): changelog — squad-gitops 권장/);
    assert.match(upLog, /갱신 동결\(비추천\): security-scan — squad-audit 권장/);

    // 3) doctor: 비추천 설치분 안내 + exit 0 (에러 아님)
    const doc = captureIO();
    assert.equal(cmdDoctor(root, doc.io), 0);
    assert.match(doc.cap.logs.join('\n'), /비추천 컴포넌트 설치됨: changelog — squad-gitops 권장/);

    // 4) uninstall: manifest 기반이라 deprecated 포함 완전 제거 (라이프사이클 영구 보장)
    const un = uninstall(root);
    assert.ok(un.removed.includes(changelogSkill), 'deprecated 자산이 uninstall로 제거 안 됨');
    assert.ok(!existsSync(join(root, changelogSkill)));
    assert.ok(!existsSync(join(root, scanSkill)));
