...
import {
  cmdDiff, cmdUpdate, cmdMigrate, cmdInstall, cmdDoctor,
...
// ── Scenario E: 삭제된 컴포넌트(fade-out 7종) 잔여 자동정리 — carve update orphan cleanup ──

test('라이프사이클 E: 구버전 설치(삭제된 7종 잔여) → update(orphan 정리·사용자수정 보존·비대상 불가침) → uninstall', () => {
  withTemp((root) => {
    seedProject(root);
    assert.equal(cmdInstall(root, captureIO().io), 0);
    // 실제 설치엔 anti-slop 팩의 clean-html 스킬이 포함된다(tombstone 외 — orphan 정리가 건드리면 안 됨).
    const cleanHtml = '.claude/skills/clean-html/SKILL.md';
    assert.ok(existsSync(join(root, cleanHtml)), 'clean-html 설치 전제 실패');

    // pre-1.5.0 설치 재현: 삭제된 memory 스킬+shim(carve 소유·미수정), 사용자가 수정한 changelog를 디스크+manifest에 심는다.
    const memSkill = '.claude/skills/memory/SKILL.md';
    const memShim = '.claude/commands/carve-memory.md';
    const clogSkill = '.claude/skills/changelog/SKILL.md';
    const memC = '# memory skill (pre-1.5.0)\n';
    const shimC = 'memory shim (pre-1.5.0)\n';
    const clogDisk = '# changelog skill — USER EDITED\n';
    mkdirSync(join(root, '.claude/skills/memory'), { recursive: true });
    mkdirSync(join(root, '.claude/skills/changelog'), { recursive: true });
    mkdirSync(join(root, '.claude/commands'), { recursive: true });
    writeFileSync(join(root, memSkill), memC);
    writeFileSync(join(root, memShim), shimC);
    writeFileSync(join(root, clogSkill), clogDisk);
    const m0 = readManifest(root) as Manifest;
    const extra: ManifestFile[] = [
      { path: memSkill, hash: hashContent(memC), assetVersion: '1.4.1' },   // 해시 일치(미수정) → 제거
      { path: memShim, hash: hashContent(shimC), assetVersion: '1.4.1' },   // 제거
      { path: clogSkill, hash: hashContent('# changelog ORIGINAL\n'), assetVersion: '1.3.0' }, // 해시 불일치(사용자 수정) → 보존
    ];
    writeManifest(root, { ...m0, files: [...m0.files, ...extra] });

    // update: removeOrphanedComponents가 carve 소유·미수정 orphan만 제거(해시 가드).
    const up = captureIO();
    assert.equal(cmdUpdate(root, up.io), 0);
    assert.ok(!existsSync(join(root, memSkill)), 'orphan memory 스킬 미정리');
    assert.ok(!existsSync(join(root, memShim)), 'orphan memory shim 미정리');
    assert.ok(!existsSync(join(root, '.claude/skills/memory')), '빈 스킬 디렉터리 미정리');
    assert.equal(read(root, clogSkill), clogDisk, '사용자 수정 changelog가 삭제/변경됨');
    assert.ok(existsSync(join(root, cleanHtml)), 'clean-html(tombstone 외·실설치) 오삭제');
    const upLog = up.cap.logs.join('\n');
    assert.match(upLog, /삭제된 컴포넌트 잔여 정리/);
    assert.match(upLog, /사용자 수정분 보존: \.claude\/skills\/changelog\/SKILL\.md/);

    // manifest 동기: 제거된 memory 경로는 빠지고, 보존분은 남는다.
    const m1 = readManifest(root) as Manifest;
    assert.ok(!m1.files.some((f) => f.path === memSkill), 'manifest에 orphan memory 잔존');
    assert.ok(!m1.files.some((f) => f.path === memShim), 'manifest에 orphan shim 잔존');
    assert.ok(m1.files.some((f) => f.path === clogSkill), '보존분이 manifest에서 빠짐');

    // doctor: 삭제된(미등재) 컴포넌트는 byId 미등재라 안내하지 않는다 + exit 0(에러 아님).
    const doc = captureIO();
    assert.equal(cmdDoctor(root, doc.io), 0);

    // uninstall: manifest 기반 완전 제거(보존분·clean-html 포함).
    uninstall(root);
    assert.ok(!existsSync(join(root, clogSkill)));
    assert.ok(!existsSync(join(root, cleanHtml)));
