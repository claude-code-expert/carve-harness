...
test('라이프사이클 E: 구버전 설치(deprecated 포함) → update(보존·동결·안내) → doctor(안내) → uninstall(완전 제거)', () => {
  withTemp((root) => {
    ...
    assert.equal(cmdInstall(root, captureIO().io), 0);
    const reviewSkill = '.claude/skills/review/SKILL.md';
    const scanSkill = '.claude/skills/security-scan/SKILL.md';
    const reviewContent = '# review skill (pre-wave-1 install)\n';
    ...
    writeFileSync(join(root, reviewSkill), reviewContent);
    writeFileSync(join(root, scanSkill), scanContent);
    const m0 = readManifest(root) as Manifest;
    const extra: ManifestFile[] = [
      { path: reviewSkill, hash: hashContent(reviewContent), assetVersion: '1.3.0' },
    ...
    const up = captureIO();
    assert.equal(cmdUpdate(root, up.io), 0);
    assert.equal(read(root, reviewSkill), reviewContent, 'update가 deprecated 자산을 건드림');
    assert.equal(read(root, scanSkill), scanContent, 'update가 deprecated 자산을 건드림');
    const upLog = up.cap.logs.join('\n');
    assert.match(upLog, /갱신 동결\(비추천\): review — squad-review 권장/);
    ...
    assert.match(doc.cap.logs.join('\n'), /비추천 컴포넌트 설치됨: review — squad-review 권장/);

    // 4) uninstall: manifest 기반이라 deprecated 포함 완전 제거 (라이프사이클 영구 보장)
    const un = uninstall(root);
    assert.ok(un.removed.includes(reviewSkill), 'deprecated 자산이 uninstall로 제거 안 됨');
    assert.ok(!existsSync(join(root, reviewSkill)));
