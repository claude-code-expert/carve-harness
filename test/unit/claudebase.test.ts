...
// ── 응답 언어 정책 ({{RESPONSE_POLICY}} 치환, --lang 프리셋) ──
test('generateClaudeBase: 기본(en-ko) 렌더는 영어요약→한글결론 정책 + 잔여 {{변수}} 없음', () => {
  const arts = generateClaudeBase(profile({ languages: ['typescript'] }));
  const base = arts.find((a) => a.path === '.claude/CLAUDE.md');
  assert.ok(base);
  assert.match(base.content, /English summary → Korean conclusion/);
  assert.match(base.content, /한글로 최종 결론/);
  assert.doesNotMatch(base.content, /\{\{\w+\}\}/); // RESPONSE_POLICY 치환 완료
});

test('generateClaudeBase: lang=ko → 한국어 단일 정책, lang=en → 영어 단일 정책', () => {
  const ko = generateClaudeBase(profile({}), { lang: 'ko' }).find((a) => a.path === '.claude/CLAUDE.md');
  assert.ok(ko);
  assert.match(ko.content, /\| 한국어 \|/);
  assert.doesNotMatch(ko.content, /English summary → Korean conclusion/);
  const en = generateClaudeBase(profile({}), { lang: 'en' }).find((a) => a.path === '.claude/CLAUDE.md');
  assert.ok(en);
  assert.match(en.content, /\| User-facing response \| English \|/);
  assert.doesNotMatch(en.content, /한글로 최종 결론/);
});
