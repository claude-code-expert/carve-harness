...
6. **절차 규율도 점검한다(Fablize).** 세 가지를 확인한다:
   - 결정 근거 — 비자명 결정에 ID(D1·D2…)와 이유·비용·탈출구가 주석에 외화됐는가.
   - 가정 — `sprint-contract.md` `## 가정` 표에 검증법과 fallback이 채워졌는가.
   - 에스컬레이션 — 스펙 밖 결함이 *선행* 보고됐는가(나중이 아니라).
   비어 있으면 해당 항목을 PARTIAL 이하로 본다.
