...
- Never run destructive migrations (drop/recreate) against production.
...
- Dependency additions or version changes (`Cargo.toml`).
- Build/config changes (`Cargo.toml`, `rust-toolchain.toml`, `build.rs`).
...
- Never run `cargo update` blindly to "fix" a build — investigate the root cause first.
- Never add `unsafe` or a new crate without rationale + approval.
- For external AI/3rd-party APIs: call asynchronously (never block the executor),
