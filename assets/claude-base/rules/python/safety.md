...
- Never run `alembic downgrade base` or edit/delete an applied Alembic migration.
...
- Never run `Base.metadata.drop_all` / destructive migrations against production.
...
- DB schema / model changes; new Alembic migration files.
- Dependency additions or version changes (`pyproject.toml`).
- Build/config changes (`pyproject.toml` build settings, mypy/ruff config).
...
- Never run `{{PKG_MANAGER}} audit fix --force` (or equivalent forced dependency rewrite).
- No new library outside the core stack without rationale + approval.
- For external AI/3rd-party APIs: call asynchronously (never block the event loop),
