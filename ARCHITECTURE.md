...
| M2 | Designer — OpenHarness 10서브시스템 슬롯 매핑 + 하네스 레벨 제안 | MS2 |
...
## post-PoC 로드맵 (SHOULD/COULD, 랭킹순)

- **v0.2 (SHOULD)**: Evaluator 서브에이전트(#1) · Sprint Contract 생성(#2) · auditor 강화(#3) · 검증훅 강화 shellcheck/tsc(#4)
- **v0.3 (COULD)**: 멀티에이전트 3병렬(#5) · Evaluator 튜닝 루프(#6) · 모델 3-Tier 라우팅(#7)
- **v0.4+ (COULD)**: COORDINATOR 메일박스(#8) · harness-audit 메타커맨드(#10) · provider 추상화(#11) · TUI(#12)
