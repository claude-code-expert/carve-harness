...
| `assets/squad/` | Squad 자산(melt-in) | 9 에이전트·10 커맨드(+squad 디스패처)·라우터/체이닝 훅. vendor 비의존 |
...
| 4 | M6·M7 | wizard(선택) + harness-architect 스킬 + installer 멱등 + uninstall + Squad 9 보존 | E2E install→doctor→uninstall, 멱등, 자산 보존 |
...
- **v1.0** 
✅: MVP(M1–M7) + 카탈로그-자산 정합.
✅ (SHOULD #1–4): Evaluator 서브에이전트 · Sprint Contract 생성 · auditor 강화(shellcheck/bash -n).
✅ (COULD #5–7): 멀티에이전트 병렬 · Evaluator 튜닝 루프 · 모델 3-Tier 라우팅.
✅ (COULD #8·#10): harness-audit · coordinator. (#11 provider · #12 TUI = carve 모델 밖, out-of-fit/보류)
✅: 토큰 효율 기본 탑재 — codesight(MCP+refresh) + LSP(cclsp MCP + 언어서버 자동설치). 모든 구성요소가 grep 대신 우선 사용.
✅: 설치 레벨 강제 플래그 `--level <minimal|standard|full>` — 프로필 자동 판정을 사용자가 수동 override(`full`은 단일언어에도 병렬·조율 스킬 설치).
✅: `carve init-claude` — CLAUDE.md 베이스라인(`.claude/CLAUDE.md`) + 언어 스택별 `.claude/rules/*`(ts/py/go/rust/java/dart/_default × 6) + 스택무관 공용 규칙 `anti-ai-slop` 생성, 루트 `CLAUDE.md` `@import` 멱등 연결. 내부 템플릿 `assets/claude-base/`.
✅ (문서): 전체 설치 매뉴얼 `INSTALL.md`(한)·`INSTALL.en.md`(영) + README 재정비(풀 설치·라이브 벤치 점수·제거). 코드 변경 없음.
- **v1.2.0** (v2.0 1차 — MS7 로드맵 M8·M9·M10):
✅ M8 라이프사이클: manifest 스키마 v2(자산 hash + `schemaVersion`) + `carve diff`/`update`/`migrate`. 사용자 수정 보존, audit 게이트, manifest-last 원자성.
✅ M9 분석 지능화: 모노레포·컨테이너 시그널 탐지 → `ProjectProfile` 확장, 시그널 가중 스코어링, `.carve-prefs.json` 선호 영속화.
✅ M10 로컬 효과 텔레메트리(opt-in): `_metrics.sh` emit(기본 OFF·네트워크 없음·secret 비기록) + `carve report` 집계.
- **v1.3.0** (운용 3대 조건 보강 + 전수 감사 패치):
✅ 샌드박스 피드백 루프: `iterate` 스킬(green까지 자율 수렴·최대 N회·최종만 보고). 계획 분리·검증: 구현 전 승인 게이트 + 계획 정량 채점(squad-plan/sprint-contract/squad-evaluator). 컨텍스트 다이어트: 40% 예산·편집파일 한정·세션 분리.
✅ 치명 결함 패치: `CLAUDE.md` 센티넬↔v1 판정 충돌(`update` 데드락) 해소, install manifest union(클린 제거), `pre-push-test` 기본값, `--level` 검증·영속, `auto-commit` 구현, `report` 0-fire 오탐 외 8영역 35건.
- **잔여**: M11(비교·증명 벤치)·M12(피드백 루프), 벤치 cross-harness 축 3(트리거)·4(컨텍스트) 라이브 측정, v1.1 절약 수치 검증(대형 fixture).
